...
import me.bokov.prog3.command.server.RoomChangedCommand;
...
public class RoomChangedCommandHandlerProviderBean implements ChatClientCommandHandlerProviderBean {
    ...
    public Collection<String> getHandledCommands() {
        return Arrays.asList("ROOM-CHANGED");
    }
    ...
    public CommandHandler<ChatClientMessageHandlingContext> getCommandHandler() {
        return (context, request) -> {
            ...
            chatUIBean.handleRoomChange(request.getData().asJsonObject().getJsonNumber("roomId").longValue());

            return ResponseBuilder.create()
                    .messageId(request.getMessageId())
                    .code(RoomChangedCommand.SUCCESS)
                    .build();
